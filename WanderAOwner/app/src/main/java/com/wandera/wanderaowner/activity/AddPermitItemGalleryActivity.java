...
public class AddPermitItemGalleryActivity extends AppCompatActivity {
    ...
    protected void onStart() {
        ...
        context = AddPermitItemGalleryActivity.this;
...
//                ImageBannerOldURL = bannerUri;
    ...
    private void setImage(Uri uri, ImageView imageView){
        ...
        bannerUri = uri;
        imageSet = true;
        Glide.with(AddPermitItemGalleryActivity.this).load(uri).into(imageView);
...
//                        uploadProgress.setText("Uploading " +taskSnapshot.getBytesTransferred()+" of "+file.available()+" "+progress+" %");
    ...
    private void saveMenu(String imagePath){
        String key = mDatabase.child(Utils.PERMIT_DIR).child(businesskey).child(categoryKey).push().getKey();
        PermitItemMapModel permitImageDataModel = new PermitItemMapModel(key,categoryKey,imagePath,businesskey);
        Map<String,Object> value = permitImageDataModel.toMap();
        Map<String,Object> childUpdates = new HashMap<>();
        childUpdates.put(key,value);
        mDatabase.child(Utils.PERMIT_DIR).child(businesskey).child(categoryKey).updateChildren(childUpdates).addOnSuccessListener(new OnSuccessListener<Void>() {
            @Override
